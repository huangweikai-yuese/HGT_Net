...
OLIST_TOP5_FEATURES = [
    "review_score",           # 历史平均评分（仅来自cutoff前已评价订单）
    "historical_order_count", # 历史订单数量
    "avg_pago",               # 平均订单金额
    "product_count",          # 产品多样性（不同产品ID数）
...
def build_customer_features(
    ...
    # 只保留已送达的订单（order_status == "delivered"）
    ...
    # 步骤1：找出cutoff前已送达的订单ID
    delivered_pre_cutoff_ids = obs_orders.loc[
        obs_orders["order_delivered_customer_date"] <= cutoff, "order_id"
    ].unique()
    
    # 步骤2：只保留这些订单的评价
    reviews_pre_cutoff = data["reviews"][
        data["reviews"]["order_id"].isin(delivered_pre_cutoff_ids)
    ].copy()
    
    # 步骤3：聚合评价分数
    if len(reviews_pre_cutoff) > 0:
        reviews_agg = reviews_pre_cutoff.groupby("order_id", as_index=False).agg(
            review_score=("review_score", "mean")
        )
    else:
        # 如果没有评价数据，创建空DataFrame避免报错
        reviews_agg = pd.DataFrame(columns=["order_id", "review_score"])
    
    # ============================================
    # 合并所有订单级特征到观测订单
    # ============================================
    obs = obs_orders.merge(payments, on="order_id", how="left")
    obs = obs.merge(items, on="order_id", how="left")
    obs = obs.merge(reviews_agg, on="order_id", how="left")
    
    obs["tiempo_estimado"] = (
        obs["order_estimated_delivery_date"] - obs["order_purchase_timestamp"]
    ).dt.days
...
def make_olist_tabular(
    ...
    df = build_customer_features(
        data_dir, observation_months, prediction_months, label_strategy=label_strategy
    )
    ...
    train_idx, val_idx = train_test_split(
        train_idx, test_size=val_size, random_state=seed, stratify=y[train_idx]
    )
